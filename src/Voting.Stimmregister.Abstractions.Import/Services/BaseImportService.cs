    ...
    where TEntity : ImportedEntity, new()
{
    ...
    protected abstract void SignEntities(TState state);
    ...
    private async Task InitializeState(TState state, TRecord firstRecord)
    {
        ...
        var cantonBfs = await _cantonBfsCache.GetBfsOfCanton(canton);
        if (cantonBfs == null)
        {
            throw new ImportException("No canton acl entry found for " + municipalityId);
        }
    ...
    {
        var entity = _mapper.MapRecordToEntity(state, record);
