...
public abstract class IncrementalPersonPayloadBuilderV3<T> : IDisposable, IIncrementalSignaturePayloadBuilder<T, PersonEntity>
{
    private readonly PersonSignaturePayloadBuilderV3 _personPayloadBuilder;
    private readonly SignaturePayloadBuilder _builder;
    private PersonEntity? _previousPerson;
    private bool _disposedValue;

    protected IncrementalPersonPayloadBuilderV3(
        string typeName,
        ObjectPool<HashBuilder> hashBuilderPool,
        IHsmCryptoAdapter hsmCryptoAdapter,
        PersonSignaturePayloadBuilderV3 personPayloadBuilder)
    {
